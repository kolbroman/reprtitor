...
TEST_QUESTIONS = [
    ...
    {"q": "На чем фокусируется внимание в технике «Принципиал»?", "options": ["На личных качествах", "На эмоциях", "На интересах, фактах и критериях"], "correct": "На интересах, фактах и критериях"},
    {"q": "Главное преимущество метода «Маленькие ходы»?", "options": ["Получить всё и сразу", "Создает динамику и снижает риск отказа", "Запутывает оппонента"], "correct": "Создает динамику и снижает риск отказа"},
    ...
    {"q": "Какая техника снижает психологический барьер?", "options": ["Три да", "Игра в одни ворота", "Принципиал"], "correct": "Три да"},
...
PRACTICUM_QUIZ = [
    {"q": "«Давайте пока не будем спорить о финальной сумме. Предлагаю для начала согласовать только график...»", "options": ["Три да", "Маленькие ходы", "Принципиал"], "correct": "Маленькие ходы"},
    ...
    {"q": "«Мы готовы снизить цену на 10%, ЕСЛИ вы заключите договор на два года...»", "options": ["Три да", "Принципиал", "Если вместо нет"], "correct": "Если вместо нет"},
    {"q": "«Давайте уберем эмоции и посмотрим на рынок. Вот статистика зарплат...»", "options": ["Маленькие ходы", "Принципиал", "Три да"], "correct": "Принципиал"},
...
TRAINING_SCENARIOS = {
    "Три да": [
        ...
        {
            "q": "Ситуация 4: Разговор с другом. Хотите пойти в кино на ужастик.",
            "options": [
        ...
        {
            "q": "Ситуация 5: Финальный аккорд. Вы получили 2 «Да». Как переходим к сути?",
            "options": [
                {"text": "Тогда предлагаю выбрать этот вариант, он как раз учитывает всё вышесказанное.", "correct": True, "feedback": "✅ Правильно: логичное завершение цепочки"},
                {"text": "Ну, раз ты согласен, давай сделаем как я хочу.", "correct": False, "feedback": "❌ Ошибка: Звучит как манипуляция"},
                {"text": "Значит, ты обязан согласиться.", "correct": False, "feedback": "❌ Ошибка: Давление"}
            ]
        }
    ],
    "Если вместо нет": [
        ...
        {
            "q": "Ситуация 2: Начальник: «Выйди на работу в воскресенье».",
            "options": [
        ...
        },
        {
            ...
            "options": [
                {"text": "Я сдам завтра черновой вариант, если финальную версию мы утвердим в среду.", "correct": True, "feedback": "✅ Правильно: изменение рамок задачи"},
    ...
    "Принципиал": [
        ...
        },
        {
            ...
            "options": [
                ...
                {"text": "Ну, я могу сделать небольшую скидку.", "correct": False, "feedback": "❌ Ошибка: Торг без обоснования"}
            ]
        },
        {
            ...
            "options": [
                ...
                {"text": "Ты ничего не понимаешь в маркетинге.", "correct": False, "feedback": "❌ Ошибка: Оскорбление"}
            ]
        },
        {
    ...
    "Игра в одни ворота": [
        ...
        },
        {
            ...
            "options": [
                ...
                {"text": "Ладно, только подпишите уже договор.", "correct": False, "feedback": "❌ Ошибка: Сдача"},
        ...
        },
        {
            ...
            "options": [
                {"text": "Мне не сложно, но мое рабочее время стоит денег. Любая работа должна быть оплачена.", "correct": True, "feedback": "✅ Правильно: защита ценности"},
...
def get_main_keyboard(user_id):
    ...
    if user.get('paid'):
        ...
        markup.add(types.KeyboardButton("📝 Тест по теории"), types.KeyboardButton("🤖 Отработка техник"))
    else:
...
def get_cases_keyboard():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    btns = [types.KeyboardButton(f"💼 {case}") for case in bot_content['cases'].keys()]
    markup.add(*btns)
    markup.add("🔙 Назад к урокам")
    return markup
...
def get_training_menu_keyboard():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    btns = [types.KeyboardButton(f"⚔️ {tech}") for tech in TRAINING_SCENARIOS.keys()]
...
def send_question(chat_id, user_id, quiz_data, mode_title):
    ...
    if index >= len(quiz_data):
        score = state['score']
        total = len(quiz_data)
        bot.send_message(chat_id, f"🏁 <b>{mode_title} завершен!</b>\nВаш результат: {score} из {total}.", 
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "Добро пожаловать в Репетитор по техникам ведения переговоров!", 
...
def handle_message(message):
    ...
    if text in ["🔙 Выход в меню", "🔙 В главное меню", "🔙 Выход из админки"]:
    ...
    if state:
        ...
        if mode == 'test':
            ...
            if text == current_q['correct']:
                ...
                bot.send_message(message.chat.id, f"❌ Ошибка. Верный ответ: {current_q['correct']}")
            else:
                bot.send_message(message.chat.id, "Пожалуйста, выберите вариант из кнопок.")
                return

            state['index'] += 1
            send_question(message.chat.id, user_id, TEST_QUESTIONS, "Тест")
            return
        ...
        if mode == 'practicum':
            ...
            if text == current_q['correct']:
                ...
                bot.send_message(message.chat.id, f"⚠️ Неверно. Правильная техника: {current_q['correct']}")
            else:
                bot.send_message(message.chat.id, "Используйте кнопки.")
                return

            state['index'] += 1
            send_question(message.chat.id, user_id, PRACTICUM_QUIZ, "Практикум")
            return
        ...
        if mode == 'training':
            ...
            if selected:
                ...
                state['index'] += 1
                send_question(message.chat.id, user_id, scenario_data, f"Тренинг: {scenario_name}")
            else:
        ...
        if mode == 'feedback':
            ...
            bot.send_message(message.chat.id, "✅ Сообщение отправлено администратору!", reply_markup=get_main_keyboard(user_id))
            for admin in ADMIN_ID:
                try: bot.send_message(admin, f"🔔 <b>Новый вопрос!</b>\nОт: @{message.from_user.username}\nТекст: {text}", parse_mode="HTML")
                except: pass
        ...
        if mode == 'admin_reply' and user_id in ADMIN_ID:
            ...
            if target_entry:
                try:
                    bot.send_message(target_entry['user_id'], f"📩 <b>Ответ администратора:</b>\n\n{text}", parse_mode="HTML")
                    bot.send_message(message.chat.id, "✅ Ответ отправлен.", reply_markup=get_main_keyboard(user_id))
                    fb_list = [m for m in fb_list if m['id'] != target_msg_id]
                    save_json(FEEDBACK_FILE, fb_list)
                except Exception as e:
                    bot.send_message(message.chat.id, f"❌ Не удалось отправить (пользователь заблокировал бот?): {e}")
            else:
                bot.send_message(message.chat.id, "Сообщение не найдено в базе.")
        ...
        if mode == 'edit_theory_content' and user_id in ADMIN_ID:
            ...
            bot.send_message(message.chat.id, f"✅ Теория «{topic}» обновлена.", reply_markup=get_main_keyboard(user_id))
            del user_states[user_id]
            admin_panel(message)
            return
        ...
        if mode == 'edit_case_content' and user_id in ADMIN_ID:
            ...
            save_json(CONTENT_FILE, bot_content)
            bot.send_message(message.chat.id, f"✅ Кейс «{case_name}» обновлен.", reply_markup=get_main_keyboard(user_id))
            del user_states[user_id]
            admin_panel(message)
            return
    ...
    if text == "📝 Тест по теории":
        ...
        bot.send_message(message.chat.id, "🚀 Определяем", reply_markup=types.ReplyKeyboardRemove())
        send_question(message.chat.id, user_id, PRACTICUM_QUIZ, "Практикум")
        ...
        bot.send_message(message.chat.id, "💳 Переведите 20 BYN на карту: 9112 3801 7366 0165 и отправьте скришот администратору: @chestachka\nЗатем нажмите кнопку подтверждения.", reply_markup=markup)
    ...
    elif text == "📚 Уроки по переговорам": bot.send_message(message.chat.id, "Выберите раздел:", reply_markup=get_lessons_keyboard())
    elif text == "📖 Теория": bot.send_message(message.chat.id, "Выберите тему:", reply_markup=get_theory_keyboard())
    elif text == "💼 Кейсы": bot.send_message(message.chat.id, "Выберите кейс:", reply_markup=get_cases_keyboard())
    elif text == "🔙 Назад к урокам": bot.send_message(message.chat.id, "Разделы:", reply_markup=get_lessons_keyboard())
        ...
        if case_name in bot_content['cases']:
            ...
            try:
                ...
                bot.send_message(message.chat.id, f"Ошибка отображения: {e}\n\nТекст: {caption}")
        ...
        if not pending:
            ...
            for uid in pending:
                ...
                mk.add(types.InlineKeyboardButton("✅ Одобрить", callback_data=f"approve_{uid}"))
                bot.send_message(message.chat.id, f"Заявка от @{uname} (ID: {uid})", reply_markup=mk)
        ...
        if not fb_list:
            ...
            bot.send_message(message.chat.id, "Входящие сообщения:", reply_markup=markup)
        ...
        bot.send_message(message.chat.id, "Выберите тему для редактирования:", reply_markup=markup)
        ...
        bot.send_message(message.chat.id, "Выберите кейс для редактирования:", reply_markup=markup)
...
def handle_callbacks(call):
    ...
    if call.data.startswith("approve_") and user_id in ADMIN_ID:
        ...
        if uid in users_db:
            ...
            save_json(DB_FILE, users_db)
            bot.edit_message_text(f"✅ Оплата для {uid} подтверждена.", call.message.chat.id, call.message.message_id)
            try: bot.send_message(int(uid), "🎉 Оплата подтверждена! Вам доступны Тесты и Практикум.\nНажмите /start для обновления меню.")
            except: pass
        ...
        if msg:
            txt = f"👤 @{msg['username']}\n📅 {msg['date']}\n\n{msg['text']}"
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("↩️ Ответить", callback_data=f"reply_{msg_id}"),
                       types.InlineKeyboardButton("🗑 Удалить", callback_data=f"del_{msg_id}"))
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("📝 Изменить текст", callback_data=f"edittheory_{topic}"))
        bot.send_message(call.message.chat.id, f"Тема: {topic}", reply_markup=markup)
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("📝 Изменить контент", callback_data=f"editcase_{case_name}"))
        bot.send_message(call.message.chat.id, f"Кейс: {case_name}", reply_markup=markup)
        ...
        bot.send_message(call.message.chat.id, f"Пришлите фото/видео/текст для «{case_name}»:")
